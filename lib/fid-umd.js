...
(function (n, r, f) {
	...
	FidUmd.prototype.createPreamble = function (config) {
		...
		// Finish the preamble
		dependencies = this.dependsProperty(config.depends, 'name', ', ');
	...
	FidUmd.prototype.createPreambleRoot = function (config) {
		...
		dependencies = this.dependsProperty(config.depends, 'root', ', ', 'r.');
	...
	FidUmd.prototype.createPreambleYui = function (config) {
		...
		dependsPassed = dependencies.join(', ');
	 ...
	 * @param string joiner Optional - Return a joined string instead of array
	 * @param string pre Optional - String to add before every result
	 * @param string post Optional - String to add after every result
	 * @return array|string
	 */
	FidUmd.prototype.dependsProperty = function (depends, property, joiner, pre, post) {
		var result;
		...
		result = depends.map(function (oneDependency) {
			return pre + oneDependency[property] + post;
		});
