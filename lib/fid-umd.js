...
(function (n, r, f) {
	 ...
	 * will not complain.
	 *
	 * @param {...string} str Strings to turn into attempts
	 * @return {string}
	 */
	FidUmd.prototype.tryTo = function (str) {
		var args, myself;
		myself = this;
		...
		args = args.map(function (arg) {
			...
			letter = myself.makeLetter(myself.counter);
			myself.counter += 1;

			while (letter === 'f' || letter === 'n' || letter === 'r') {
				letter = myself.makeLetter(myself.counter);
				myself.counter += 1;
			}

			str = '\ttry { ' + arg + ' } catch (' + letter + ') {}';
